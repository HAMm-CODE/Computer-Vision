...
%            x and w?
% Yes, the output differs between runs because the convolution filters are randomly initialized using randn.%
%380   508    10%
%To calculate the size of y we use h=H−5+1 w=W−5+1 where the spatial dimensions are reduced due to valid convolution with a 
%5×5 filter and the third dimension corresponds to the number of filters.
%x dictates the size of the input image and w dictates the third dimension
%corresponds to the number of filters.and filter used
...
%Compared to the unpadded output y, y_pad has larger spatial dimensions. With padding = 2 and a 
%5×5 filter, the output size becomes 𝐻×𝑊×10, matching the input height and width.
...
% Without repmat, the filter would not match the input depth, and the convolution would fail
% The filter extracts high-frequency features such as edges and contours in all directions.     
...
% The network would reduce to a single linear transformation regardless of depth. 
% Non-linearities allow CNNs to model complex patterns, learn hierarchical features, and solve problems that are not linearly separable.
...
%Max-pooling reduces the spatial resolution of feature maps while preserving strong activations,
%The stride parameter controls the step size of the pooling window and therefore determines the amount of downsampling.
...
%In an ideal case, the histograms of blob and non-blob scores would be clearly separated, 
%with minimal overlap and few values between the classification thresholds. 
%In the current result without pre-processing, the distributions overlap significantly, 
%leading to ambiguous scores and reduced detection performance.
